        ...
        attach: function (context, settings) {
            $(once('paragraph--type--smart', 'body', context)).each(
                function () {
                    ...
                    var url = drupalSettings.path.baseUrl + 'personalised_content/reactions/' + settings.smartContentParagraphs.nid;
                    getLocation();
                    var data = {
                            ...
                            success: function (results) {
                                ...
                                $('.paragraph--type--smart').each(
                                    function () {
                                        ...
                                        $.each(
                                            removables, function (key, value) {
                                                $.each(
                                                    value, function (key, paragraphId) {
                                                        if ($(main_paragraph).find(".paragraph--" + paragraphId).length > 0) {
                                                            show_default = false;
                                                            $(".paragraph--" + paragraphId).show();
                                                        }
                                                    }
                                                );
                                            }
